...
/* 메인 게시물 내용 박스(줄마다 나눠서 디자인하였음) css */
.main_content .row_1st{
	display:flex;
	border-bottom:1px solid gray;
	background-color:  rgb(250, 238, 228);
}
.main_content .row_1st .row_1st_num{
	width:50px;
	text-align: center;
	background-color:  rgb(250, 238, 228);
}
.main_content .row_1st .row_1st_title{
	width:700px;
	text-align: center;
	border:none;
	background-color:  rgb(250, 238, 228);
...
.main_content .row_2nd{
	display:flex;
	background-color:lightgray;
}

.main_content .row_2nd .row_2nd_img{
	width:50px;
	margin-left:20px;
}
.main_content .row_2nd .row_2nd_name{
	width:500px;
	border:none;
	background-color:lightgray;
}

.main_content .row_3rd{
	display:flex;
}
.main_content .row_3rd .row_3rd_content{
	width:100%;
	height:600px;
...
/* 푸터에서 다음페이지와 이전페이지로 이동하는 버튼 박스 css */
.footer_buttons>div{
	height:120px;
	border:5px solid #e0f3fa;
}
.footer_buttons a{
	height:100%;
	width: 100%;
	...
	<div class="wrapper">
		...
		<div class="main_content">
			<form name="newUpdate"
				...
				<div>
					<div class="row_1st">
						<div class="row_1st_num"><%=notice.getNum()%></div>
						<input  class="row_1st_title" name="subject" class="form-control"	value="<%=notice.getSubject()%>" >
						<div><%=notice.getRegist_day()%></div>
					</div>
					
					<div class="row_2nd">
						<img class="row_2nd_img" src="./resources/image/example1.png">    <!-- 프로필 사진 넣을 곳 -->
						<input class="row_2nd_name" name="name" class="form-control"	value="<%=notice.getName()%>">
					</div>
					
					
					<div class="row_3rd">
						<textarea class="row_3rd_content" name="content" class="form-control" cols="50" rows="5"><%=notice.getContent()%></textarea>
					</div>
					...
					<a href="./Popular_BoardListAction.do?pageNum=<%=nowpage%>"	class="btn" style="width:60px; background-color:darkcyan; color:white;"> 목록</a>
					<a href="#"	class="btn btn-info" style="margin-left:100px;"> 공유</a>
					</div>
				</div>
			</form>
		</div>
			...
			<div class="footer_buttons">
				<div>
					<div class="next_page_button">
						...
						<a class="text-dark"
							style="text-decoration-line: none;"
							href="./Popular_BoardViewAction.do?num=<%=num_next%>&Popular_pageNum=<%=1%>">다음 페이지<%=subject_next%></a>
					</div>
					<div class="before_page_button">
						<%-- Popular_pageNum=<%=1%> 부분은 쓸모없는 변수(더미 코드)이다 --%>
						<a class="text-dark"
							style="text-decoration-line: none;"
							href="./Popular_BoardViewAction.do?num=<%=num_before%>&Popular_pageNum=<%=1%>">이전 페이지<%=subject_before%>2222</a>
					</div>
				</div>
